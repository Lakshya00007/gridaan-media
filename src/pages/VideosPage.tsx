        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading
            ...
            : videos.map(article => {
                ...
                return (
                  <Link key={article.id} to={`/article/${article.slug}`} className="group bg-card rounded-3xl overflow-hidden border border-border hover:shadow-md transition-all hover:-translate-y-1 flex flex-col h-full">
                    ...
                    <div className="p-5 flex flex-col justify-between flex-1">
                      ...
                      <div className="flex items-center gap-3 text-[10px] text-text-secondary font-medium mt-auto">
                        ...
                        <span>·</span>
                        <span className="flex items-center gap-0.5"><Eye className="w-3 h-3" />{(views / 1000).toFixed(1)}K views</span>
                        <span>·</span>
                        <span className="flex items-center gap-0.5"><Clock className="w-3 h-3" />{readingTime}m read</span>
                      </div>
                    </div>
                  </Link>
                );
