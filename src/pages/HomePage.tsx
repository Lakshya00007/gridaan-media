    ...
    <div className="min-h-screen">
          ...
          <div className="grid lg:grid-cols-2 gap-8 items-center">
            ...
            <div>
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-5 leading-[1.1]">
                Where <span className="bg-linear-to-r from-[#327CFA] via-[#7FB0FF] to-[#003CC6] bg-clip-text text-transparent">innovation</span> meets insight
              </h1>
              ...
              <div className="flex flex-wrap gap-3 mb-8">
                <Link to="/trending" className="inline-flex items-center gap-2 px-6 py-3 bg-linear-to-r from-[#327CFA] to-[#003CC6] text-white rounded-xl font-semibold shadow-sm shadow-[#327CFA]/10 hover:shadow-lg hover:shadow-[#327CFA]/30 transition-all hover:-translate-y-0.5">
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link to="/trending" className="hidden md:flex items-center gap-1 text-[#327CFA] dark:text-[#94A3B8] font-medium text-sm hover:gap-2 transition-all">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
          {categories.slice(0, 10).map(cat => (
            <Link key={cat.id} to={`/category/${cat.slug}`} className="group p-4 bg-[#0B1224] dark:bg-[#0B1224]/50 rounded-xl border border-[#1E293B]/50 dark:border-[#1E293B]/50 hover:shadow-lg hover:shadow-gray-200/50 dark:hover:shadow-gray-900/50 transition-all hover:-translate-y-1">
              ...
              <h3 className="font-semibold text-sm text-[#F8FAFC] dark:text-white group-hover:text-[#327CFA] dark:group-hover:text-[#94A3B8] transition-colors">{cat.name}</h3>
              ...
              {['All', 'Technology', 'AI & Machine Learning', 'Programming', 'Web Development', 'Cloud Computing'].map(cat => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all ${
                    activeCategory === cat
                      ? 'bg-[#327CFA] text-white shadow-lg shadow-[#327CFA]/20 dark:shadow-[#003CC6]/30'
          ...
          <aside className="space-y-6">
                ...
                {loading ? (
                  ...
                  trending.slice(0, 5).map((article, i) => (
                    <div key={article.id} className="flex gap-3 py-3 border-b border-[#1E293B] dark:border-[#1E293B] last:border-0">
                      ...
                      <div>
                        <Link to={`/article/${article.slug}`} className="text-sm font-medium text-[#F8FAFC] dark:text-white hover:text-[#327CFA] dark:hover:text-[#94A3B8] transition-colors line-clamp-2">
                          {article.title}
                        </Link>
                        <span className="text-xs text-[#94A3B8] mt-0.5 block">{article.readingTime} min read</span>
                      </div>
                    </div>
                  ))
                )}
            ...
            <div className="bg-[#0B1224] dark:bg-[#0B1224]/50 rounded-2xl border border-[#1E293B]/50 dark:border-[#1E293B]/50 p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link to="/tutorials" className="text-xs text-[#327CFA] dark:text-[#94A3B8] font-medium">View all →</Link>
              </div>
            ...
            {/* Newsletter Mini */}
            <div className="bg-linear-to-br from-[#327CFA] to-[#003CC6] rounded-2xl p-6 text-white">
              ...
              <Link to="/" className="block text-center py-2.5 bg-[#0B1224] text-[#327CFA] rounded-xl text-sm font-semibold hover:bg-[#0B1224] transition-colors">
                Subscribe Free
              </Link>
            ...
            <div className="bg-[#0B1224] dark:bg-[#0B1224]/50 rounded-2xl border border-[#1E293B]/50 dark:border-[#1E293B]/50 p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {['React', 'AI', 'Python', 'JavaScript', 'Docker', 'AWS', 'TypeScript', 'Node.js', 'Kubernetes', 'GraphQL', 'Rust', 'Go', 'DevOps', 'Security'].map(tag => (
                  <Link key={tag} to={`/search?q=${tag}`} className="px-3 py-1.5 bg-[#0B1224] dark:bg-[#0B1224] text-gray-600 dark:text-[#94A3B8] rounded-full text-xs font-medium hover:bg-[#0B1224] hover:text-[#327CFA] dark:hover:bg-[#0B1224]/30 dark:hover:text-[#94A3B8] transition-colors">
                    {tag}
                  </Link>
                ))}
              </div>
      ...
      <section className="bg-[#0B1224] dark:bg-[#0B1224]/50 py-12 border-y border-[#1E293B]/50 dark:border-[#1E293B]/50">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link to="/tutorials" className="hidden md:flex items-center gap-1 text-[#327CFA] dark:text-[#94A3B8] font-medium text-sm hover:gap-2 transition-all">
